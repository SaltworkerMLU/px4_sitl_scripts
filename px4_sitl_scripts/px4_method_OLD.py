...
import numpy as np
from geometry_msgs.msg import Vector3 # x=roll, y=pitch, z=yaw (in radians)
...
class JerkLimitedSetpointSmoother:
    """
    3rd-order (jerk-limited) reference trajectory generator.
    Matches PX4's VelocitySmoothing / MPC trajectory planner behaviour.
    
    State: [position, velocity, acceleration]
    Limits: max_vel, max_acc, max_jerk
    
    PX4 defaults (X500):
        MPC_JERK_AUTO = 8.0  m/s³
        MPC_ACC_HOR   = 3.0  m/s²
        MPC_XY_VEL_MAX= 5.0  m/s
    """
    def __init__(self, max_vel=3.0, max_acc=3.0, max_jerk=3.0):
    ...
    def reset(self, pos, vel=0.0, acc=0.0):
        self.pos = pos
        self.vel = vel
        self.acc = acc

    def update(self, pos_target: float, dt: float) -> tuple:
        error = pos_target - self.pos

        # Compute stopping distance accounting for BOTH current vel AND acc
        # Time to reduce acc to zero: t_a = |acc| / max_jerk
        # Distance covered while killing acceleration first
        t_a = abs(self.acc) / (self.max_jerk + 1e-6)
        vel_after_acc_kill = self.vel + self.acc * t_a
        stop_dist = (vel_after_acc_kill ** 2) / (2.0 * self.max_acc + 1e-6) \
              + abs(self.vel * t_a + 0.5 * self.acc * t_a**2)

        # Bang-bang jerk: accelerate or brake based on whether we'd overshoot
        if abs(error) < 1e-3 and abs(self.vel) < 1e-2:
        ...
        going_right_dir = (error > 0 and self.vel >= 0) or (error < 0 and self.vel <= 0)

        if going_right_dir and abs(error) > stop_dist:
            jerk = self.max_jerk * np.sign(error)      # keep accelerating
        else:
            jerk = -self.max_jerk * np.sign(self.vel + 1e-10)  # start braking

        self.acc = float(np.clip(self.acc + jerk * dt, -self.max_acc, self.max_acc))
        self.vel = float(np.clip(self.vel + self.acc * dt, -self.max_vel, self.max_vel))
...
class XYController:
    ...
    def __init__(
        ...
        max_vel_xy: float = 5.0, # 12.0,       # m/s – MPC_XY_VEL_MAX
        max_tilt: float = np.radians(45.0),  # rad – MPC_TILTMAX_AIR
        max_thr_xy: float = 1.0, # 0.9,
    ...
    def update(
        ...
        vel_ff: np.ndarray,   # [vx_ff, vy_ff]  NED [m/s]
        dt:     float,
    ) -> np.ndarray:
        """
        ...
        vel_sp = self.Kp_pos * (pos_sp - pos) + vel_ff
...
class ZController:
    ...
    def update(self, z_sp: float, z: float, vz: float, v_ff: float, dt: float) -> float:
        """        thrust_d = (
            self.Kp_vel * vz_err
            + self.Ki_vel * self._vel_int
...
class PositionControllerNode(Node):
    ...
    def __init__(self):
        ...
        # ── Setpoints ──────────────────────────────────────────────────
        self.pos_sp = np.array([2.0, 1.0, -2.0])   # NED [m]: x=2, y=1, alt=2m
        self.yaw_sp = 0.0                            # rad
        ...
        self.angle_pub    = self.create_publisher(Vector3,
                                '/fmu/out/vehicle_attitude_euler', 10)
        
        # -- SMOOTH CRIMINAL
        self.smoothers = [
        JerkLimitedSetpointSmoother(max_vel=3.0, max_acc=3.0, max_jerk=8.0),  # x
        JerkLimitedSetpointSmoother(max_vel=3.0, max_acc=3.0, max_jerk=8.0),  # y
        JerkLimitedSetpointSmoother(max_vel=2.0, max_acc=2.0, max_jerk=5.0),  # z
        ]
        self._smoothers_initialised = False

        # Initialise at current position (call after first odometry message)
        for i, s in enumerate(self.smoothers):
            s.reset(self.pos[i])
    ...
    def odometry_cb(self, msg: VehicleOdometry):
        ...
        # Initialise smoothers exactly once, on first real odometry
        if not self._smoothers_initialised:
            for i, s in enumerate(self.smoothers):
                s.reset(self.pos[i])
            self._smoothers_initialised = True
    ...
    def timer_cb(self):
        ...
        if self.counter >= 10 and self._smoothers_initialised:
    ...
    def _publish_attitude_setpoint(self):
        #-- SMOOTHENING ENHANCE
        #smooth_sp, smooth_sv, _ = np.array([
        #    self.smoothers[i].update(self.pos_sp[i], self.dt)[0]
        #    for i in range(3)
        #    ])
        smooth_x,  vel_ff_x,  _ = self.smoothers[0].update(self.pos_sp[0], self.dt)
        smooth_y,  vel_ff_y,  _ = self.smoothers[1].update(self.pos_sp[1], self.dt)
        smooth_z,  vel_ff_z,  _ = self.smoothers[2].update(self.pos_sp[2], self.dt)

        # Pass vel feedforward into controllers — add a vel_ff parameter
        thr_xy = self.xy_ctrl.update(
            pos_sp = np.array([smooth_x, smooth_y]),
            pos    = self.pos[:2],
            vel    = self.vel[:2],
            vel_ff = np.array([vel_ff_x, vel_ff_y]),   # <-- add this
            dt     = self.dt,
        )
        #thr_xy = self.xy_ctrl.update(smooth_sp[:2], self.pos[:2], self.vel[:2], self.dt)
        thr_z  = self.z_ctrl.update(smooth_z,  self.pos[2],  self.vel[2], 0,  self.dt)
        ...
        # PX4 prioritises Z: vertical t        hrust is set first, then
        ...
        # ── Log at 2 Hz ────────────────────────────────────────────────
        if True: # self.counter % (self.RATE_HZ // 2) == 0:
            ...
            self.get_logger().info(
                ...
                f'thr={thrust_body_z:.3f}'
            )
            #roll, pitch, yaw = self._q_to_euler(q_wxyz)

            # -- Publish euler angles
            msg_angles = Vector3()
            msg_angles.x = np.rad2deg(roll)
            msg_angles.y = np.rad2deg(pitch)
            msg_angles.z = np.rad2deg(yaw)

            self.angle_pub.publish(msg_angles)
