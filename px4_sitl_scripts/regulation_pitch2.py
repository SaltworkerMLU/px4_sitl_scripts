...
# Top plot: pitch input signal
axes[0].plot(t_uniform, np.degrees(pitch_uniform), color='orange', label='PX4 Pitch (deg)')
axes[0].set_title('Pitch Angle Input from PX4 (used as TF input)')
axes[0].set_xlabel('Time (s)')
axes[0].set_ylabel('Pitch (degrees)')
axes[0].legend()
axes[0].grid(True)

# Bottom plot: x-position comparison
axes[1].plot(t_uniform, x_pos_tf, 'b-', label='TF output (driven by real pitch)')
axes[1].plot(t_slice, x_pos, color='orange', label='PX4 actual x-position')
axes[1].set_title('Option C: X-position — TF driven by real pitch vs PX4 actual')
axes[1].set_xlabel('Time (s)')
axes[1].set_ylabel('X-axis position (m)')
