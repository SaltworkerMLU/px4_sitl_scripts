...
# -- Yes
# Load CSV
df = pd.read_csv("step_1_y_avg/data.csv")
...
for i in range(20):
    l = 4705 + i*2000
    data[i,:] = df['/fmu/out/vehicle_odometry/position[1]'].values[l:l+1000]
for i in range(20):
    l = 3705 + i*2000
    data2[i,:] = df['/fmu/out/vehicle_odometry/position[1]'].values[l:l+1000]
...
plt.title('Step Response of Y-axis position (using PX4-based script)')
plt.xlabel('Time (s)')
plt.ylabel('Y-position (m)')
plt.legend(['Transfer function', 'Px4-Autopilot 0m->1m (average of 20 runs)', 'Px4-Autopilot 1m->0m (average of 20 runs)'])
