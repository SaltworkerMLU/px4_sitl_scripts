...
# -- Yes
# Load CSV
df = pd.read_csv("step_1_z_avg/data.csv")

#print(df.iloc[0])
#print(df['/fmu/out/vehicle_odometry/position[0]'])

data = np.zeros((20, 1000))
data2 = np.zeros((20, 1000))

for i in range(20):
    l = 581 + i*2000
    data[i,:] = df['/fmu/out/vehicle_odometry/position[2]'].values[l:l+1000]
for i in range(20):
    l = 1581 + i*2000
    data2[i,:] = df['/fmu/out/vehicle_odometry/position[2]'].values[l:l+1000]

data_avg = 2+np.mean(data, axis=0)
data_avg2 = -1-np.mean(data2, axis=0)
...
plt.plot(t, data_avg)
plt.plot(t, data_avg2)
plt.axhline(1.0, color='k', linestyle='--', linewidth=0.8, label='Setpoint')
plt.title('Step Response of Z-axis position (using PX4-based script)')
plt.xlabel('Time (s)')
plt.ylabel('Z-position (m)')
plt.legend(['Transfer function', 'Px4-Autopilot 1m->2m (average of 20 runs)', 'Px4-Autopilot 2m->1m (average of 20 runs)'])
